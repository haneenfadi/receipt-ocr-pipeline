...
class ReceiptQuerySystem:
    ...
    def get_database_schema(self) -> str:
        ...
        DATABASE SCHEMA
        Table: receipts
        id INTEGER PK | store_name TEXT | receipt_number TEXT | date TEXT (free text, no STRFTIME) | currency TEXT | taxes TEXT | total_amount TEXT (CAST AS REAL for math) | image_path TEXT | created_at TIMESTAMP
    ...
    def text_to_sql(self, user_question: str) -> str:
        """
...
if __name__ == "__main__":
    system = ReceiptQuerySystem(db_name="receipts.db")

    # ex: "كم فاتوره عندي بشهر4 سنة 2022 "
    # ex: "شو أغلى فاتوره عندي"
    question = "كم صرفت في مطعم ليالي الشام؟"
   
    print("Question:", question)

    sql = system.text_to_sql(question)
    print("\nGenerated SQL:")
    print(sql)
    print("*" * 50)
    
    results = system.execute_sql(sql)
    print("\nexecute_sql Results:")
    print(results)
    print("*" * 50)

    answer = system.format_answer(question, results)
    print("\n format_answer:")
    print(answer)
    print("*" * 50)

    print("\nFormatted Answer:")
    formatted_answer = system.format_sql_answer(question, results)

    print("*" * 50)
    print("\nCombined Answer:")
    combined = system.combined_answer(question, results)
    print(combined)
